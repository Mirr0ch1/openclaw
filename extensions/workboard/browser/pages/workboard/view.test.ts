...
describe("renderWorkboard", () => {
  ...
  it("disposes its mounted dashboard when card details close", () => {
    const { state, container, renderView } = createWorkboardView();
    state.detailCardId = "card-1";
    state.cards = [createWorkboardCard({ sessionKey: "agent:main:dashboard-panel-close" })];
    const mount = vi.mocked(workboardTestHost().host.components.mountDashboard);
    renderView();
    expect(mount).toHaveBeenCalledWith(
      expect.any(HTMLElement),
      expect.objectContaining({ sessionKey: "agent:main:dashboard-panel-close" }),
    );
