  ...
  switch (input.action) {
    ...
    case "get_accessibility_tree": {
      if (input.windowRef || input.query || input.depth !== undefined || input.maxElements) {
        throw new Error(
          "COMPUTER_UNSUPPORTED_ACTION: CUA Driver 0.22.0 exposes get_accessibility_tree only as unfiltered desktop discovery; use get_window_state for a window tree",
        );
      }
