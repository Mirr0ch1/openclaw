...
handoff log for the recovery outcome. The helper collects diagnostic-only
[triage](/cli/triage) after recovery and readiness checks finish, before releasing
its install lease. Update, recovery, and diagnostic child exits are logged
separately; only the helper completion marks the end of the handoff. Successful
triage cannot turn an update failure into success, retry the update, or authorize
another restart.
