...
async function runManagedServiceManagerBoundary(
  ...
  await fs.writeFile(
    ...
    export async function waitForGatewayUpdateRecovery(expectedVersion, expectedBuildId) {
      ...
      ${options?.diagnosticReadFailure ? `{ const db = new (require("node:sqlite").DatabaseSync)(${JSON.stringify(stateDatabasePath)}); db.exec("ALTER TABLE gateway_restart_sentinel RENAME COLUMN thread_id TO unreadable_thread_id"); db.close(); }` : ""}
        ...
        expect
          ...
          .toBe(
            options?.helperExitCode ??
              (options?.systemdHandoffFailure || options?.updaterSignal
                ? 1
                : (options?.updaterExitCode ?? 7)),
          );
...
describe("managed service update handoff", () => {
  ...
  itUnix.each([
    ...
    { kind: "systemd", updaterSignal: true },
    { kind: "launchd", updaterSignal: true },
  ] as const)(
