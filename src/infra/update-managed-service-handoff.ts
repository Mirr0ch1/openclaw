...
function recordUpdateHandoffOutcome(reason, restored, childStatus, expectedRevision) {
  ...
  if (status === "error" && (childStatus !== "skipped" || restored === false)) {
  ...
  try {
    runManagedUpdateLeaseTransaction(db, () => {
      ...
      if ((payload?.status === "error" || (childStatus === "skipped" && payload?.status === "skipped")) && typeof restored !== "boolean" &&
        (!triageFailure || payload.doctorHint === params.triageHint)) {
      ...
      if (payload) {
        ...
        payload = { ...payload, status: childStatus && typeof restored !== "boolean" ? payload.status : childStatus === "skipped" && restored ? "skipped" : status };
        delete payload.continuation;
      } else {
        payload = fallbackPayload;
      }
      if (isFailedUpdatePayload(payload)) {
  ...
  try {
    ...
    const { updaterOutput, outputOverflow } = exit;
      ...
      if (exit.code === ${MANAGED_SERVICE_UPDATE_UNSAFE_EXIT_CODE}) {
        ...
        if (!childStatus || result.status === "error" || !params.nonFailureSkippedReasons.includes(result.reason)) {
          triageFailure = { reason: result?.reason || "managed-service-handoff-failed" };
        }
